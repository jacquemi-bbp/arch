...
pyarch -v convert --config-file-path ../Config/batch_convert.ini

printf "\nPIPELINE INFO: Convert QuPath project metadata to a pandas dataframes\n"
pyarch -v convert-qupath-project --qupath-project-path $QUPATH_PROJECT_PATH --output-path $RESULT_PATH
...
pyarch -v density-per-layer --config-file-path ../Config/batch_density_layer_merged.ini

printf "\nPIPELINE INFO: Compute cell densities per Layer with L2 and L3 distinguished\n"
pyarch -v density-per-layer --config-file-path ../Config/batch_density_layer_distinguish.ini
