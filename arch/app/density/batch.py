...
from arch.utilities import concat_dataframe, stereology_exclusion
...
@click.option("--compute-per-layer", is_flag=True, default=False)
@click.option("--compute-per-depth", is_flag=True, default=True)
@click.option(
    "--image-to-exlude-path", help="exel files that contains the list of image to exclude (xlsx).", required=False
)

def cmd(config_file_path, visualisation_flag, save_plot_flag,
         image_to_exlude_path, compute_per_layer, compute_per_depth):
    """
    The batch density click command
    """
